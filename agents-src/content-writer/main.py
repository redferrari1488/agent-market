...
def build_prompts(pillar: str) -> tuple[str, str]:
    ...
    parts.append(
        "Пиши живым человеческим языком, не как AI-копирайтер. Категорически избегай: "
        "штампованных антитез «не X, а Y», «дело не в…, а в…»; рубленых односложных "
        "предложений ради драмы («Делайте. Потом. Лучше.»); пафосных инфоцыганских "
        "выводов. Спокойная интонация реального человека, который пишет в свой канал."
    )
    if EXAMPLES:
        parts.append(
...
def strip_markdown(text: str) -> str:
    ...
    # Нормализация типографики: модель любит длинные/средние тире, живые
    # каналы пишут обычным дефисом. Детерминированно, в отличие от промпта.
