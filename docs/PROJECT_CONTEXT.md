...
- **Чистка мёртвого кода** (детали в блоке ниже 2026-06-08): 16 файлов, build-verified, запушено. Ждёт деплоя.
- **Стратегия распространения** `drafts/hireon-distribution-plan.docx` — переписана 3 раза до v3 (малый бюджет, творчество+соцсети+AI, dogfooding как главный рычаг). Юр-рамка РФ-2026 (проверено web): ФЗ-72 — реклама в Instagram/FB/X/Reddit/Discord/LinkedIn запрещена; ЕРИР-маркировка + сбор 3% на платное; НПД/422-ФЗ — деньги только со своих агентов.
- **Маркетинг-арсенал** `drafts/marketing/` + `drafts/launch-post.md`: 16 TG-постов, лид-магнит 2ГИС (48 ответов, .docx), аутрич-скрипт 2ГИС, партнёрский оффер, 6 видео-сценариев, лонгрид VC/Habr, SEO-тексты. Код: SEO-лендинги `/resheniya/[slug]`.
- **КРИТИЧНО — gating рекламы по готовности агентов.** Аудит выявил: 2 из 3 «локомотивов» маркетинга — waitlist, купить нельзя: `review-responder-2gis` (ответы 2ГИС, проблема — парсинг 2GIS HTML) и `telegram-support-bot` (бот, silent logs). Готовы к продаже только `content-writer`, `competitor-monitor`, `news-digest-bot` (`LANDING_FEATURED_SLUGS` в `src/app/page.tsx`). Карта валидности по каждому файлу/посту — `drafts/marketing/00-СТАТУС-арсенала.md`. SEO-лендинги в коде урезаны до копирайтера (2ГИС/бот → 404, `dynamicParams=false`, до фиксов).

**Честный вывод сессии:** широкая реклама рано. Сначала E2E готового агента (купить своей картой → настроить → убедиться что работает у клиента; в `todo.md` это открытый блокер), потом постинг ВАЛИДНОГО контента (см. 00-СТАТУС). Арсенал готов и ждёт.

**Дальше:** (1) задеплоить с ноута — поднимет чистку кода + копирайтер-лендинг; (2) доделать waitlist-агентов (2ГИС: парсинг; бот: silent logs); (3) E2E готового агента; (4) постинг по 00-СТАТУС.
