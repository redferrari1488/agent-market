...
- **2026-06-09 (review-responder-2gis):** `resolve_public_key()` выдирал ключ 2GIS reviews-API из HTML главной `2gis.ru` регэкспом `"key":"([a-f0-9-]+)"`. 2GIS сменил разметку → 0 совпадений → `RuntimeError` каждый цикл → агент мёртв (это и была «проблема парсинга 2GIS HTML»). Fix: убрать скрейп, использовать долгоживущий публичный read-only ключ `6e7e1929-4ea9-4a5d-8c05-d601860389bd` (зашит в веб-клиент 2GIS, проверен живым запросом — HTTP 200) как дефолт + env-override `TWOGIS_PUBLIC_KEY` + явная ошибка при HTTP 401/403. Reviews API: `GET https://public-api.reviews.2gis.com/2.0/branches/{branch_id}/reviews?key=…`, ответ `{"meta":{...},"reviews":[...]}`. Поля отзыва: `rating` (плоское число), `text`, автор в объекте `user` (`user.name`/`user.first_name`), `official_answer`. ПРАВИЛО: не скрейпить динамический HTML ради API-ключа — это хрупко; публичные ключи API выноси в константу + env-override.
- **2026-06-09 (telegram-support-bot диагностика):** «молчание» бот не имел детерминированного бага в коде. Проверка фактами (клон upstream father-bot + прогон патчей): дефолт `available_text_models[0]=gpt-3.5-turbo` (НЕ уходит в vision-ветку `message_handle`), дефолт `current_chat_mode="assistant"` совпадает с обрезанным `entrypoint.sh` chat_modes.yml (нет KeyError), все 5 regex-патчей entrypoint матчат upstream HEAD и компилируются. `start.py` уже чинит silent-logs. Остаток — рантайм (streaming claude/OpenRouter в openai 0.28; Markdown v1 parse), верифицируется только прогоном `docker compose up` с реальным токеном + mongo + диалогом. ПРАВИЛО: father-bot — clone-at-build, upstream-код не в репо; для диагностики клонируй его и читай реальные bot.py/openai_utils/config/database, не строй гипотез по обёртке.
